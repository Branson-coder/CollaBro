...
export default function AuthPage() {
  ...
  const switchMode = (m) => {
    setMode(m); setError(''); setLogin(''); setEmail(''); setUsername(''); setPassword(''); setConfirm('')
  }
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      mode === 'login' ? await doLogin({ login, password }) : await register({ email, username, password })
      navigate('/board')
    } catch (err) {
    ...
    <div className="auth-root">
      ...
      <div className="auth-left">
        ...
        <div className="auth-tagline">
          <h1>Task Management<br /> For Teams <br/> All in one place.</h1>
        </div>
        <div className="auth-features">
          {[
            'Kanban board with real-time sync',
            'Team roles, permissions, assignments',
            'File attachments and due dates',
              ...
              {mode === 'login' ? (
                <>
                  <div className="auth-field">
                    ...
                    <input autoFocus autoComplete="username" value={login} onChange={(e) => setLogin(e.target.value)} className="auth-field-input" required />
                  </div>
                  <div className="auth-field">
                    <span className="auth-field-label">Password</span>
                    <input type="password" autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} className="auth-field-input" required />
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div className="auth-field">
                    ...
                    <input autoComplete="username" value={username} onChange={(e) => setUsername(e.target.value)} className="auth-field-input" required />
                  </div>
                  <div className="auth-field">
                    <span className="auth-field-label">Password</span>
                    <input type="password" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} className="auth-field-input" required />
                  </div>
                  <div className="auth-field">
